...
const ignoredPatterns = [
  ...rootLevelIgnoredFiles.map(
    (fileName) => new RegExp(`^${escapeForRegex(path.join(__dirname, fileName))}$`),
  ),
  new RegExp(`^${escapeForRegex(path.join(__dirname, 'functions'))}(?:[\\\\/].*)?$`),
];
