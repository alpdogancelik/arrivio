...
export default function IssueScreen() {
    ...
    try {
      if (Platform.OS !== 'web') {
        ...
        if (!permission.granted) {
          Alert.alert(t('issue:attach'), t('issue:photoPermission'));
          return;
        }
      }
      ...
      const err = mapApiError(error);
      Alert.alert(t('issue:attach'), err.message || t('issue:attachFailed'));
    }
  };
  ...
  const submit = () => {
    ...
    mutation.mutate({ description: clean, category, photo: selectedPhoto ?? undefined });
  };
        ...
        <ThemedView style={styles.card}>
          ...
          >
            ...
            <ThemedText style={styles.attachText}>
              {selectedPhoto ? t('issue:changePhoto') : t('issue:attach')}
            </ThemedText>
          </Pressable>
          ...
          {selectedPhoto ? (
            <View style={styles.photoPreview}>
              ...
              <View style={styles.photoPreviewContent}>
                <ThemedText style={styles.photoPreviewTitle}>{t('issue:photoAttached')}</ThemedText>
                <ThemedText style={styles.photoPreviewMeta} numberOfLines={1}>
                  {selectedPhoto.fileName || t('issue:selectedPhoto')}
                </ThemedText>
                <Pressable onPress={() => setSelectedPhoto(null)} style={styles.photoRemoveButton}>
                  <ThemedText style={styles.photoRemoveText}>{t('issue:removePhoto')}</ThemedText>
                </Pressable>
              </View>
            </View>
          ) : null}
